...
function qcld_woo_map_bargin_product(){
    ...
    $variation_id   = isset( $_POST['qcld_woo_map_variation_id'] )      ? trim(sanitize_text_field($_POST['qcld_woo_map_variation_id'])) : '';
...
function qcld_woo_bargin_product_price(){
    ...
    $product_id     = isset( $_POST['qcld_woo_map_product_id'] )        ? trim(sanitize_text_field($_POST['qcld_woo_map_product_id'])) : '';
    $variation_id   = isset( $_POST['qcld_woo_map_variation_id'] )      ? trim(sanitize_text_field($_POST['qcld_woo_map_variation_id'])) : '';
    $customer_price = isset( $_POST['price'] )                          ? trim(sanitize_text_field($_POST['price'])) : '';
...
function qcld_woo_bargin_product_modal_price(){
    ...
    $sale_price_include_tax = $actual_price;

    // Price include tax
    if( ! wc_prices_include_tax() ) {
        $sale_price_include_tax = wc_get_price_including_tax( $product, array('price' => $actual_price ) );
    }
    ...
    if(!empty($minimum_price_check) && ($multi_currency == 'simple')){
        $minimum_price          = $product->get_meta( 'qcld_minimum_acceptable_price' );
        $sale_price_include_tax = $product->get_meta( 'qcld_minimum_acceptable_price' );
    }elseif(!empty($minimum_price_check) && ($multi_currency == 'multiple')){
        $discount_percent_price =  $minimum_price_check;
        $minimum_price = ( $actual_price * $discount_percent_price / 100 );
        $sale_price_include_tax =  ( $sale_price_include_tax * $discount_percent_price / 100 );
    ...
    $response = array('html' => $html, 'confirm' => $confirm, 'sale_price_include_tax' => $sale_price_include_tax, 'sale_price' => $sale_price );
...
if (!function_exists('qcld_woo_bargin_product_add_to_cart')){
    function qcld_woo_bargin_product_add_to_cart(){
        ...
        $quantity           = isset( $_POST['product_qty'] ) ? sanitize_text_field($_POST['product_qty']) : 1;
